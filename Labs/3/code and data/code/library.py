...
def compute_node_centrality(graph):
    ...
    w_degrees = [round(float(w_degree)/(len(graph.vs)-1),5) for w_degree in w_degrees]
...
def print_top10(feature_names, clf, class_labels):
    ...
    for i, class_label in enumerate(class_labels):
        coefs = clf.coef_[i]
        top10_indices = sorted(range(len(coefs)), key=lambda i: coefs[i], reverse=True)[:10]
        top10_features = [feature_names[i] for i in top10_indices]
        print 'Top 10 features for class ' + class_label + ': ' + ', '.join(top10_features)
